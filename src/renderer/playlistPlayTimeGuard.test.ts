...
describe("playlist play time guard", () => {
  it("allows draft typing and validates Play time only when the input is committed", () => {
    ...
    expect(appSource).toContain("formatPlayTimeMinimumStatus(minimumSeconds, language)");
    ...
    expect(appSource).toContain("function showInvalidPlayTimeWarning(minimumSeconds: number): void");
    expect(appSource).toContain("const message = formatPlayTimeMinimumStatus(minimumSeconds, language);");
    expect(appSource).toContain("alertOnly: true");
    expect(appSource).toContain("onInvalidPlayTime={showInvalidPlayTimeWarning}");
    expect(appSource).toContain("onInvalidPlayTime(minimumSeconds);");
    expect(appSource).toContain("resetPlayTimeDraft(item.id);");
    expect(appSource).toContain("Play時間はフェードアウトより長く設定してください（最小:");
    expect(appSource).not.toContain("Play時間はループ開始位置＋フェードアウトより長く設定してください（最小:");
  ...
  it("validates Time mode against fade length when Fade changes", () => {
    expect(appSource).toContain("getPlaylistMinimumDurationMs(track, rule)");
    expect(appSource).not.toContain('type PlayTimeMinimumReason = "fade" | "loop-marker";');
    expect(appSource).not.toContain("function getPlayTimeMinimumReason(track: BgmTrack, rule: PlaylistRule): PlayTimeMinimumReason");
    ...
    expect(appSource).toContain("onInvalidPlayTime(getMinimumPlayTimeSeconds(track, nextRule));");
    ...
    expect(appSource).toContain("onInvalidPlayTime(playTimeViolation);");
  });
});
